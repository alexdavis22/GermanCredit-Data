...
```
**NOTE: I'm not sure how to make the h2o code reproducible. Despite setting a seed my code returns a slightly different output every time. I'm seeing AUC ranges of anywhere from 0.80-0.83 in the knitted html document. Conventional accuracy is ranging from about 0.80-0.84. I'm also seeing that every time the model runs it is minimizing false positives while still correctly predicting Good credit risks the majority of the time, which is exactly what a business stakeholder would want in this scenario.**
