...
GLM.train <- train(Class~InstallmentRatePercentage+Checking+Credit.History+OtherInstallmentPlans,  data=trainDF, method="glm", family="binomial")
...
vif(GLM.train1)
```
**Housing and Property variables appear to suffer from multicollinearity... Loan.Purpose as well depending on if threshold is >3 or >4. Those variables have already been removed from the model above using step AIC method though. Let's build another model and reduce our predictor variables to the Step AIC selection used above.**

```{r}
set.seed(24)
GLM.train2 <- glm(formula = Class~InstallmentRatePercentage+Checking+Credit.History+OtherInstallmentPlans, family = binomial(link = "logit"), data = trainDF)
summary(GLM.train2)

# We may end up wanting to tweak these thresholds, since we know there is a higher cost of a false positive when you approve a loan for a bad credit risk compared to a false negative of denying a loan to a good credit risk. 
GLM.predict.2 <- predict(GLM.train2, newdata = testDF, type = "response")
GLM.predict.2[GLM.predict.2>=0.5] <- 1
GLM.predict.2[GLM.predict.2<0.5] <- 0

# Create factor vectors
actual <- testDF$Class 
predicted <- factor(GLM.predict.2, levels = c(0,1), labels = c("Bad", "Good"))
...
# ROC plot and calculating AUC
plot(roc(trainDF$Class, GLM.train2$fitted.values))

auc(trainDF$Class, GLM.train2$fitted.values)

plot(roc(testDF$Class, GLM.predict.2))

auc(testDF$Class, GLM.predict.2)
```
**At these thresholds we have a lot of false positive predictions with very few false negative predictions, and a model that is thrown out of balance. We also now have a lower AUC of only 0.6144 on our test data. Let's adjust the thresholds and see if the model performs better. NOTE: this code outputs an AUC of 0.6326 after knitting this to an html despite setting my seed in the code chunk, and I have no idea why. Sorry!**

```{r}
set.seed(25)

GLM.predict.2 <- predict(GLM.train2, newdata = testDF, type = "response")
GLM.predict.2[GLM.predict.2>=0.70] <- 1
GLM.predict.2[GLM.predict.2<0.70] <- 0
...
auc(testDF$Class, GLM.predict.2)
```
**After tweaking our thresholds we end up with an AUC of 0.6869. We still avoid classifying false positives the majority of the time while also correctly classifying a good credit risk the majority of the time. But, let's try building some other models and see if we can more accurately predict our response variable. NOTE: this code outputs an AUC of 0.6483 after knitting this to an html despite setting my seed in the code chunk, and I have no idea why. Sorry!**
...
```{r}
set.seed(29)
randomforest.model3 <- randomForest(Class~., data = balanced.trainGermanCredit.stratified2, method = "class", mtry = 4, ntree = 5000, nodesize = 180, maxnodes = 6)
...
varImpPlot(randomforest.model3)
...
**We're able to increase our accuracy slightly to 0.7486, and our AUC has also increased to 0.714. This is a notably better result than the optimized decision tree model, and this model also has a great balance between bias and variance.**
...
# The model outputs in probability rather than classifcation, so I need to set a cutoff threshold
...
print(mytune)
```
**This code is still a work in progress since I'm not familiary with the mlr package yet, but we were able to optimize the XGBoost model to a mean test accuracy of 0.7982, which is a significant improvement over the random forest built built previously.**
...
```
**This confusion matrix doesn't exactly line up with the mean test accuracy calculation provided previously, but these are the highest accuracy and balanced accuracy measurements out of any model so far. The 95% confidence intervals places this model in the range of 72.38% - 81.44% accuracte, which is above the baseline performance of this dataset. The overall accuracy is calculated to be 77.14%. Our AUC is calculated at 0.7325, which is also the highest AUC calculated of any model. The model does still have a fairly high number of false positives though. It may be worth tweakin the model further to try and reduce that number.**

**NOTE: These accuracy measurements are also not matching up with my knitted html document despite set seed in all code chunks... not sure how to fix this issue**

###Final Model Choice

**I think that, at least based on how I built and optimized these models, the random forest model is the clear choice as the model I would present to a business stakeholder. The optimized version of the random forest model has an almost perfect balance between specificity and sensitivity, and its sensitivity calculation far exceeds any of the other models at 0.7238, while still having a specificity rating of 0.7592. It also has the highest 95% confidence interval of any of the models with a range of 0.70 to 0.7932. This means that at worst the model is just as  accurate as the baseline option of the bank approving all loan applicants (which would be the correct assessment 70% of the time based on the data set), and typically will be more accurate than the baseline. The model also balances the data set to account for the higher cost to the bank that false positive predictions carry compared to a false negative.**

**We can show that the most important factors that impact someone's credit worthiness are checking account balance, requested loan duration, credit history and request loan amount. The other variables in the data set have less impact on the model's fit. So, intuitively, someone who has credit history all paid and all at this bank, has a checking account with a balance greater than 200, and are requesting a loan with a short duration and a small amount would be considered a very safe investment for the bank, and vice versa if we select the opposite factors from those variables.**

**The XGBoost has obvious merit as well, but I think it needs further optimizing before it can be presented confidently as the best model.**
